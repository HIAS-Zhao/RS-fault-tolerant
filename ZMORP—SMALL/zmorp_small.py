"""ZMORP-SMALL 对外接口：只输入模型即可完成保护、注错与恢复。
...
本文件只是薄封装：全部位级逻辑来自同目录 ``zmorp_codec.py``
（即 ``fault_tolerance_tools/codecs/zmorp.py`` 的原样副本），注错器
...
class ZMORPSmall:
    ...
    三个方法均直接接收 ``torch.nn.Module``，返回原始实现的统计对象：
    ``protect``/``recover`` 返回 ``ZMORPStats``，``inject`` 返回注错的
    张量个数。
    """
...
if __name__ == "__main__":
    ...
    print("ZMORPSmall interface self-test passed.")
